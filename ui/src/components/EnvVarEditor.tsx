...
export function EnvVarEditor({
  ...
  async function sealRow(index: number) {
    ...
    const name = window.prompt(t("envVar.secretNamePrompt"), suggested)?.trim();
    if (!name) return;
    ...
    try {
      ...
      setSealError(error instanceof Error ? error.message : t("envVar.failedToCreateSecret"));
    }
  }
  ...
  return (
    <div className="space-y-1.5">
        ...
        return (
          <div key={index} className="flex items-center gap-1.5">
            <input
              ...
              placeholder={t("envVar.keyPlaceholder")}
              value={row.key}
            ...
            >
              <option value="plain">{t("envVar.typePlain")}</option>
              <option value="secret">{t("envVar.typeSecret")}</option>
            </select>
            {row.source === "secret" ? (
              <>
                <select
                ...
                >
                  <option value="">{t("envVar.selectSecretPlaceholder")}</option>
                  {secrets.map((secret) => (
                ...
                <button
                  ...
                  title={t("envVar.createSecretTooltip")}
                >
                  {t("envVar.new")}
                </button>
              </>
              ...
              <>
                <input
                  ...
                  placeholder={t("envVar.valuePlaceholder")}
      ...
      <p className="text-[11px] text-muted-foreground/60">
        {t("envVar.automaticInjectionHint")}
      </p>
    </div>
  );
}
