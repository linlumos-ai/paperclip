...
export function ExecutionWorkspaceCloseDialog({
  ...
  const closeWorkspace = useMutation({
    ...
    onSuccess: (workspace) => {
      ...
      pushToast({
        title: currentStatus === "cleanup_failed" ? t("workspaceClose.workspaceCloseRetried") : t("workspaceClose.workspaceClosed"),
        tone: "success",
      });
      onOpenChange(false);
      onClosed?.(workspace);
    ...
    onError: (error) => {
      pushToast({
        title: t("workspaceClose.failedToClose"),
      ...
      <DialogContent className="max-h-[85vh] overflow-y-auto sm:max-w-2xl">
        ...
        {readinessQuery.isLoading ? (
          <div className="flex items-center gap-2 rounded-xl border border-border bg-background px-4 py-3 text-sm text-muted-foreground">
            ...
            {t("workspaceClose.checkingSafety")}
          </div>
          ...
          <div className="space-y-4">
            ...
            {blockingIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceClose.blockingIssues")}</h3>
            ...
            {readiness.git ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceClose.gitStatus")}</h3>
            ...
            {otherLinkedIssues.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceClose.otherLinkedIssues")}</h3>
                <div className="space-y-2">
            ...
            {readiness.runtimeServices.length > 0 ? (
              <section className="space-y-2">
                <h3 className="text-sm font-medium">{t("workspaceClose.attachedRuntimeServices")}</h3>
                <div className="space-y-2">
            ...
            <section className="space-y-2">
              <h3 className="text-sm font-medium">{t("workspaceClose.cleanupActions")}</h3>
              <div className="space-y-2">
            ...
            {currentStatus === "cleanup_failed" ? (
              <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 px-4 py-3 text-sm text-muted-foreground">
                {t("workspaceClose.cleanupFailedRetry")}
              </div>
            ) : null}
            ...
            <div className="text-xs text-muted-foreground">
              {t("workspaceClose.lastChecked", { time: formatDateTime(new Date()) })}
            </div>
          </div>
        ) : null}
