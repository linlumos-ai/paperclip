...
function useIssueColumnLabels(): Record<InboxIssueColumn, string> {
  ...
  return {
    status: t("issues.columnStatus"),
    id: t("issues.columnId"),
    assignee: t("issues.columnAssignee"),
    project: t("issues.columnProject"),
    workspace: t("issues.columnWorkspace"),
    parent: t("issues.columnParentIssue"),
    labels: t("issues.columnTags"),
    updated: t("issues.columnLastUpdated"),
  };
}
...
function useIssueColumnDescriptions(): Record<InboxIssueColumn, string> {
  const { t } = useTranslation();
  return {
    status: t("issues.columnDescStatus"),
    id: t("issues.columnDescId"),
    assignee: t("issues.columnDescAssignee"),
    project: t("issues.columnDescProject"),
    workspace: t("issues.columnDescWorkspace"),
    parent: t("issues.columnDescParentIssue"),
    labels: t("issues.columnDescTags"),
    updated: t("issues.columnDescLastUpdated"),
  };
}

export function issueActivityText(issue: Issue, t?: (key: string, opts?: Record<string, string | number>) => string): string {
  const ago = timeAgo(issue.lastActivityAt ?? issue.lastExternalCommentAt ?? issue.updatedAt);
  return t ? t("issues.updatedAgo", { time: ago }) : `Updated ${ago}`;
}
...
export function InboxIssueTrailingColumns({
  ...
  const userLabel = assigneeUserName ?? formatAssigneeUserLabel(issue.assigneeUserId, currentUserId) ?? t("issues.userDefault");
          ...
          return (
            <span key={column} className="min-w-0 truncate text-xs text-muted-foreground" title={parentTitle ?? undefined}>
              {parentIdentifier ? (
                ...
                <span className="italic">{t("issues.subIssue")}</span>
              )}
            </span>
          );
        }
