  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="overflow-hidden rounded-lg border border-border/70">
      <CollapsibleTrigger className="flex w-full items-center justify-between px-3 py-2 text-left">
        <div>
          <p className="text-sm font-medium">{t("routines.variables")}</p>
          <p className="text-xs text-muted-foreground">
            {t("routines.variablesDescription")}
          </p>
        </div>
          ...
          <div key={variable.name} className="p-4">
            <div className="mb-3 flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                {t("routines.variablesPromptDescription")}
              </span>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <div className="space-y-1.5 md:col-span-2">
                <div className="flex items-center justify-between gap-3">
                  <Label className="text-xs">{t("common.defaultValue")}</Label>
                ...
                {variable.type === "textarea" ? (
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="__unset__">{t("common.noDefault")}</SelectItem>
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routines.variablesOptions")}</Label>
                      <Input
                        ...
                        placeholder={t("routines.variablesOptionsPlaceholder")}
                      />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs">{t("routines.variablesDefaultOption")}</Label>
                      <Select
                      ...
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={t("common.noDefault")} />
                        </SelectTrigger>
                        <SelectContent>
                          <SelectItem value="__unset__">{t("common.noDefault")}</SelectItem>
                  ...
                  <Input
                    ...
                    placeholder={variable.type === "number" ? "42" : t("common.defaultValue")}
                  />
                )}
              </div>
            </div>
    ...
    <>
      <div className="flex items-center justify-between gap-2 rounded-lg border border-dashed border-border/70 px-3 py-2 text-xs text-muted-foreground">
        ...
        <button
          ...
          aria-label={t("routines.variablesHelpAriaLabel")}
        >
      ...
      <Dialog open={helpOpen} onOpenChange={setHelpOpen}>
        <DialogContent className="sm:max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("routines.variablesDialogTitle")}</DialogTitle>
            <DialogDescription>
              {t("routines.variablesDialogDescription")}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-5 text-sm">
            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routines.variablesCustomSectionTitle")}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.variablesCustomSectionDesc")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-muted-foreground">
                <li>{t("routines.variablesNameRule")}</li>
                <li>{t("routines.variablesTypeRule")}</li>
                <li>{t("routines.variablesSameNameRule")}</li>
              </ul>
            </section>

            <section className="space-y-2">
              <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                {t("routines.variablesBuiltinSectionTitle")}
              </h3>
              <p className="text-muted-foreground">
                {t("routines.variablesBuiltinSectionDesc")}
              </p>
              <div className="overflow-hidden rounded-lg border border-border/70">
                <table className="w-full text-left text-xs">
                  <thead className="bg-muted/40 text-muted-foreground">
                    <tr>
                      <th className="px-3 py-2 font-medium">{t("common.placeholder")}</th>
                      <th className="px-3 py-2 font-medium">{t("common.example")}</th>
