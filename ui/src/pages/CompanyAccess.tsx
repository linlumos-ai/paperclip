...
const permissionLabelKeys: Record<PermissionKey, string> = {
  "agents:create": "company.accessPage.permission.agentsCreate",
  "users:invite": "company.accessPage.permission.usersInvite",
  "users:manage_permissions": "company.accessPage.permission.usersManagePermissions",
  "tasks:assign": "company.accessPage.permission.tasksAssign",
  "tasks:assign_scope": "company.accessPage.permission.tasksAssignScope",
  "tasks:manage_active_checkouts": "company.accessPage.permission.tasksManageActiveCheckouts",
  "joins:approve": "company.accessPage.permission.joinsApprove",
  "environments:manage": "company.accessPage.permission.environmentsManage",
};

function formatGrantSummary(member: CompanyMember, t: (key: string) => string) {
  if (member.grants.length === 0) return t("company.accessPage.noExplicitGrants");
...
export function CompanyAccess() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      { label: selectedCompany?.name ?? t("company.accessPage.breadcrumbCompany"), href: "/dashboard" },
      { label: t("company.accessPage.breadcrumbSettings"), href: "/company/settings" },
      { label: t("company.accessPage.breadcrumbAccess") },
    ]);
  ...
  const updateMemberMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({
        title: t("company.accessPage.toast.memberUpdated"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("company.accessPage.toast.failedToUpdateMember"),
        body: error instanceof Error ? error.message : t("company.accessPage.toast.unknownError"),
        tone: "error",
      });
    },
  });
  ...
  const approveJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("company.accessPage.toast.joinRequestApproved"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("company.accessPage.toast.failedToApproveJoinRequest"),
        body: error instanceof Error ? error.message : t("company.accessPage.toast.unknownError"),
        tone: "error",
      });
    },
  ...
  const rejectJoinRequestMutation = useMutation({
    ...
    onSuccess: async () => {
      await refreshAccessData();
      pushToast({
        title: t("company.accessPage.toast.joinRequestRejected"),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("company.accessPage.toast.failedToRejectJoinRequest"),
        body: error instanceof Error ? error.message : t("company.accessPage.toast.unknownError"),
        tone: "error",
      });
    },
  ...
  const archiveMemberMutation = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      pushToast({
        title: t("company.accessPage.toast.memberRemoved"),
        body:
          result.reassignedIssueCount > 0
            ? t("company.accessPage.toast.assignedIssuesCleaned", {
                count: result.reassignedIssueCount,
                plural: result.reassignedIssueCount === 1 ? "" : "s",
              })
            : undefined,
        tone: "success",
      });
    ...
    onError: (error) => {
      pushToast({
        title: t("company.accessPage.toast.failedToRemoveMember"),
        body: error instanceof Error ? error.message : t("company.accessPage.toast.unknownError"),
        tone: "error",
      });
    },
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("company.accessPage.selectCompany")}</div>;
  }

  if (membersQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("company.accessPage.loading")}</div>;
  }

  if (membersQuery.error) {
    const message =
      membersQuery.error instanceof ApiError && membersQuery.error.status === 403
        ? t("company.accessPage.errorNoPermission")
        : membersQuery.error instanceof Error
          ? membersQuery.error.message
          : t("company.accessPage.errorFailedToLoad");
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("company.accessPage.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("company.accessPage.subtitle", { name: selectedCompany?.name ?? "" })}
        </p>
      </div>

      {access && !access.currentUserRole && (
        <div className="rounded-xl border border-amber-500/40 px-4 py-3 text-sm text-amber-200">
          {t("company.accessPage.instanceAdminMessage")}
        </div>
      )}
      ...
      <section className="space-y-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold">{t("company.accessPage.humans")}</h2>
          </div>
          <p className="max-w-3xl text-sm text-muted-foreground">
            {t("company.accessPage.manageHumanMemberships")}
          </p>
        </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold">{t("company.accessPage.pendingHumanJoins")}</h3>
                <p className="text-sm text-muted-foreground">
                  {t("company.accessPage.pendingHumanJoinsDesc")}
                </p>
              </div>
              <Badge variant="outline">{t("company.accessPage.pendingBadge", { count: pendingHumanJoinRequests.length })}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  title={
                    ...
                    t("company.accessPage.unknownRequester")
                  }
                  subtitle={
                    request.requesterUser?.email ||
                    request.requestEmailSnapshot ||
                    request.requestingUserId ||
                    t("company.accessPage.noEmail")
                  }
                  context={
                    request.invite
                      ? request.invite.humanRole
                        ? t("company.accessPage.inviteContextWithRole", { joinTypes: request.invite.allowedJoinTypes, role: request.invite.humanRole })
                        : t("company.accessPage.inviteContext", { joinTypes: request.invite.allowedJoinTypes })
                      : t("company.accessPage.inviteUnavailable")
                  }
                  detail={t("company.accessPage.submitted", { date: new Date(request.createdAt).toLocaleString() })}
                  approveLabel={t("company.accessPage.approveHuman")}
                  rejectLabel={t("company.accessPage.rejectHuman")}
        ...
        <div className="overflow-hidden rounded-xl border border-border">
          <div className="grid grid-cols-[minmax(0,1.5fr)_120px_120px_minmax(0,1.2fr)_180px] gap-3 border-b border-border px-4 py-3 text-xs font-medium uppercase tracking-wide text-muted-foreground">
            <div>{t("company.accessPage.userAccount")}</div>
            <div>{t("company.accessPage.role")}</div>
            <div>{t("company.accessPage.status")}</div>
            <div>{t("company.accessPage.grants")}</div>
            <div className="text-right">{t("company.accessPage.action")}</div>
          </div>
          {members.length === 0 ? (
            <div className="px-4 py-8 text-sm text-muted-foreground">{t("company.accessPage.noMemberships")}</div>
          ) : (
            members.map((member) => {
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{t("company.accessPage.dialog.editTitle")}</DialogTitle>
            <DialogDescription>
              {t("company.accessPage.dialog.editDesc", { name: (editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId) ?? "" })}
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
            <div className="space-y-5">
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("company.accessPage.dialog.companyRole")}</span>
                  <select
                  ...
                  >
                    <option value="">{t("company.accessPage.unset")}</option>
                ...
                <label className="space-y-2 text-sm">
                  <span className="font-medium">{t("company.accessPage.dialog.membershipStatus")}</span>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2"
                    value={draftStatus}
                    onChange={(event) =>
                  ...
                  >
                    <option value="active">{t("company.accessPage.dialog.statusActive")}</option>
                    <option value="pending">{t("company.accessPage.dialog.statusPending")}</option>
                    <option value="suspended">{t("company.accessPage.dialog.statusSuspended")}</option>
                  </select>
                </label>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <h3 className="text-sm font-medium">{t("company.accessPage.dialog.grants")}</h3>
                  <p className="text-sm text-muted-foreground">
                    {t("company.accessPage.dialog.grantsDesc")}
                  </p>
                </div>
                <div className="rounded-lg border border-border px-3 py-3">
                  <div className="text-sm font-medium">{t("company.accessPage.dialog.implicitGrantsFromRole")}</div>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? t("company.accessPage.dialog.implicitGrantsIncluded", { role: HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] })
                      : t("company.accessPage.dialog.noImplicitGrants")}
                  </p>
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("company.accessPage.dialog.implicitGrantHint", { role: draftRole ? HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole] : t("company.accessPage.dialog.selected") })}
                          </span>
                        ) : null}
                        {draftGrants.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("company.accessPage.dialog.explicitGrantHint")}
                          </span>
                        ) : null}
                      </span>
                    </label>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingMemberId(null)}>
              {t("company.accessPage.dialog.cancel")}
            </Button>
            <Button
              onClick={() => {
                if (!editingMember) return;
            ...
            >
              {updateMemberMutation.isPending ? t("company.accessPage.dialog.saving") : t("company.accessPage.dialog.saveAccess")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("company.accessPage.dialog.removeTitle")}</DialogTitle>
            <DialogDescription>
              {t("company.accessPage.dialog.removeDesc", { name: memberDisplayName(removingMember, t) })}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ? t("company.accessPage.dialog.checkingAssignedIssues")
                    : t("company.accessPage.dialog.assignedIssuesCount", { count: assignedIssues.length, plural: assignedIssues.length === 1 ? "" : "s" })}
                </div>
              </div>

              {assignedIssues.length > 0 ? (
                <div className="space-y-2">
                  <div className="text-sm font-medium">{t("company.accessPage.dialog.issueReassignment")}</div>
                  <select
                    className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                    value={reassignmentTarget}
                    onChange={(event) => setReassignmentTarget(event.target.value)}
                  >
                    <option value="__unassigned">{t("company.accessPage.dialog.leaveUnassigned")}</option>
                    {activeReassignmentUsers.length > 0 ? (
                      <optgroup label={t("company.accessPage.dialog.humans")}>
                    ...
                    {activeReassignmentAgents.length > 0 ? (
                      <optgroup label={t("company.accessPage.dialog.agents")}>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {t("company.accessPage.dialog.moreIssues", { count: assignedIssues.length - 6, plural: assignedIssues.length - 6 === 1 ? "" : "s" })}
                      </div>
                    ) : null}
                  </div>
                </div>
          ...
          <DialogFooter>
            ...
            >
              {archiveMemberMutation.isPending ? t("company.accessPage.dialog.removing") : t("company.accessPage.dialog.removeMember")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function memberDisplayName(member: CompanyMember | null, t: (key: string) => string) {
  if (!member) return t("company.accessPage.thisMember");
