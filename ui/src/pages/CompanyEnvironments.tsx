...
function SupportMark({ supported }: { supported: boolean }) {
  ...
  return supported ? (
    ...
    <span className="text-muted-foreground">{t("company.env.no")}</span>
  );
}
...
export function CompanyEnvironments() {
  ...
  const environmentMutation = useMutation({
    ...
    onSuccess: async (environment) => {
      ...
      pushToast({
        title: editingEnvironmentId ? t("company.env.environmentUpdated") : t("company.env.environmentCreated"),
        body: t("company.env.environmentReady", { name: environment.name }),
        tone: "success",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("company.env.failedToSaveEnvironment"),
        body: error instanceof Error ? error.message : t("company.env.environmentProbeFailed2"),
        tone: "error",
      });
    },
  });
  ...
  const environmentProbeMutation = useMutation({
    ...
    onSuccess: (probe, environmentId) => {
      ...
      pushToast({
        title: probe.ok ? t("company.env.environmentProbePassed") : t("company.env.environmentProbeFailed"),
        body: probe.summary,
    ...
    onError: (error, environmentId) => {
      ...
      setProbeResults((current) => ({
        ...
        [environmentId]: {
          ...
          summary: error instanceof Error ? error.message : t("company.env.environmentProbeFailed2"),
          details: null,
        },
      }));
      pushToast({
        title: t("company.env.environmentProbeFailed"),
        body: error instanceof Error ? error.message : t("company.env.environmentProbeFailed2"),
        tone: "error",
      });
    },
  ...
  const draftEnvironmentProbeMutation = useMutation({
    ...
    onSuccess: (probe) => {
      pushToast({
        title: probe.ok ? t("company.env.draftProbePassed") : t("company.env.draftProbeFailed"),
        body: probe.summary,
        tone: probe.ok ? "success" : "error",
      });
    },
    onError: (error) => {
      pushToast({
        title: t("company.env.draftProbeFailed"),
        body: error instanceof Error ? error.message : t("company.env.draftProbeFailed2"),
        tone: "error",
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("company.env.selectCompany")}</div>;
  }
    ...
    return (
      <div className="max-w-3xl space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("company.env.title")}</h1>
        </div>
        <div className="rounded-md border border-border px-4 py-4 text-sm text-muted-foreground">
          {t("company.env.environmentsDisabled")}
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6" data-testid="company-settings-environments-section">
      <div className="space-y-2">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("company.env.pageDescription")}
        </p>
      </div>

      <div className="space-y-4 rounded-md border border-border px-4 py-4">
        <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("company.env.adapterSupportNote")}
        </div>

        <div className="overflow-x-auto">
          <table className="w-full min-w-[34rem] text-left text-xs">
            <caption className="sr-only">{t("company.env.title")}</caption>
            <thead className="border-b border-border text-muted-foreground">
              <tr>
                <th className="py-2 pr-3 font-medium">{t("company.env.adapter")}</th>
                <th className="px-3 py-2 font-medium">{t("company.env.local")}</th>
                <th className="px-3 py-2 font-medium">{t("company.env.ssh")}</th>
                {sandboxSupportVisible ? (
                  <th className="px-3 py-2 font-medium">{t("company.env.sandbox")}</th>
                ) : null}
              </tr>
            </thead>
        ...
        <div className="space-y-3">
          {(environments ?? []).length === 0 ? (
            <div className="text-sm text-muted-foreground">{t("company.env.noEnvironmentsYet")}</div>
          ) : (
                ...
                >
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-1">
                      ...
                      {environment.driver === "ssh" ? (
                        <div className="text-xs text-muted-foreground">
                          {typeof environment.config.host === "string" ? environment.config.host : t("company.env.sshHost")} ·{" "}
                          {typeof environment.config.username === "string" ? environment.config.username : t("company.env.user")}
                        </div>
                        ...
                        <div className="text-xs text-muted-foreground">
                          {(() => {
                            ...
                            return `${displayName} ${t("company.env.sandboxProvider")}${summary ? ` · ${summary}` : ""}`;
                          })()}
                        </div>
                      ) : (
                        <div className="text-xs text-muted-foreground">{t("company.env.runsOnThisHost")}</div>
                      )}
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                      {environment.driver !== "local" ? (
                        ...
                        >
                          {environmentProbeMutation.isPending
                            ? t("company.env.testing")
                            : environment.driver === "ssh"
                              ? t("company.env.testConnection")
                              : t("company.env.testProvider")}
                        </Button>
                      ) : null}
                      <Button
                        size="sm"
                      ...
                      >
                        {isEditing ? t("company.env.editing") : t("company.env.edit")}
                      </Button>
                    </div>
                  </div>
                  {probe ? (
        ...
        <div className="border-t border-border/60 pt-4">
          <div className="mb-3 text-sm font-medium">
            {editingEnvironmentId ? t("company.env.editEnvironment") : t("company.env.addEnvironment")}
          </div>
          <div className="space-y-3">
            <Field label={t("company.env.nameLabel")} hint={t("company.env.nameHint")}>
              <input
            ...
            <Field label={t("company.env.descriptionLabel")} hint={t("company.env.descriptionHint")}>
              <input
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                type="text"
            ...
            <Field label={t("company.env.driverLabel")} hint={t("company.env.driverHint")}>
              <select
                className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
              ...
              >
                <option value="ssh">{t("company.env.ssh")}</option>
                {sandboxCreationEnabled || environmentForm.driver === "sandbox" ? (
                  <option value="sandbox">{t("company.env.sandbox")}</option>
                ) : null}
                <option value="local">{t("company.env.local")}</option>
              </select>
            </Field>

            {environmentForm.driver === "ssh" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("company.env.hostLabel")} hint={t("company.env.hostHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("company.env.portLabel")} hint={t("company.env.portHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="number"
                    min={1}
                ...
                <Field label={t("company.env.usernameLabel")} hint={t("company.env.usernameHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("company.env.remoteWorkspacePathLabel")} hint={t("company.env.remoteWorkspacePathHint")}>
                  <input
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    type="text"
                ...
                <Field label={t("company.env.privateKeyLabel")} hint={t("company.env.privateKeyHint")}>
                  <div className="space-y-2">
                    <select
                      className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                    ...
                    >
                      <option value="">{t("company.env.noSavedSecret")}</option>
                      {(secrets ?? []).map((secret) => (
                ...
                <Field label={t("company.env.knownHostsLabel")} hint={t("company.env.knownHostsHint")}>
                  <textarea
                    className="h-32 w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-xs font-mono outline-none"
                ...
                <div className="md:col-span-2">
                  <ToggleField
                    label={t("company.env.strictHostKeyCheckingLabel")}
                    hint={t("company.env.strictHostKeyCheckingHint")}
            ...
            {environmentForm.driver === "sandbox" ? (
              <div className="grid gap-3 md:grid-cols-2">
                <Field label={t("company.env.providerLabel")} hint={t("company.env.providerHint")}>
                  <select
                    className="w-full rounded-md border border-border bg-transparent px-2.5 py-1.5 text-sm outline-none"
                ...
                <div className="md:col-span-2 space-y-3">
                  ...
                  {selectedSandboxSchema ? (
                    ...
                    <div className="rounded-md border border-border/60 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
                      {t("company.env.noAdditionalConfig")}
                    </div>
                  )}
                </div>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {environmentMutation.isPending
                  ? editingEnvironmentId
                    ? t("company.env.saving")
                    : t("company.env.creating")
                  : editingEnvironmentId
                    ? t("company.env.saveEnvironment")
                    : t("company.env.createEnvironment")}
              </Button>
              {editingEnvironmentId ? (
                <Button
                  size="sm"
              ...
              {environmentForm.driver !== "local" ? (
                ...
                >
                  {draftEnvironmentProbeMutation.isPending ? t("company.env.testing") : t("company.env.testDraft")}
                </Button>
              ) : null}
              {environmentMutation.isError ? (
                <span className="text-xs text-destructive">
                  {environmentMutation.error instanceof Error
                    ...
                    : t("company.env.failedToSaveEnvironment")}
                </span>
              ) : null}
