...
function ExportPreviewPane({
  ...
  if (!selectedFile || content === null) {
    return (
      <EmptyState icon={Package} message={t("company.exportPage.noPreview")} />
    );
  }
      ...
      <div className="min-h-[560px] px-5 py-5">
        {parsed ? (
          ...
          <div className="rounded-lg border border-border bg-accent/10 px-4 py-3 text-sm text-muted-foreground">
                        {t("company.exportPage.binaryPreview")}
          </div>
        )}
      </div>
    </div>
...
export function CompanyExport() {
  ...
  const exportPreviewMutation = useMutation({
    ...
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("company.exportPage.exportFailed"),
        body: err instanceof Error ? err.message : t("company.exportPage.failedToLoadExport"),
      });
    },
  });
  ...
  const downloadMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      pushToast({
        tone: "success",
        title: t("company.exportPage.exportDownloaded"),
        body: t("company.exportPage.exportedAsZip", { count: resultCheckedFiles.size, name: result.rootPath }),
      });
    },
    onError: (err) => {
      pushToast({
        tone: "error",
        title: t("company.exportPage.exportFailed"),
        body: err instanceof Error ? err.message : t("company.exportPage.failedToBuildExport"),
      });
    },
  });
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={Package} message={t("company.exportPage.selectCompany")} />;
  }
  ...
  if (!exportData) {
    return <EmptyState icon={Package} message={t("company.exportPage.loadingData")} />;
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-4 text-sm">
            <span className="font-medium">
              {t("company.exportPage.companyExport", { name: selectedCompany?.name ?? "Company" })}
            </span>
            <span className="text-muted-foreground">
              {t("company.exportPage.fileSelected", { count: selectedCount, total: totalFiles })}
            </span>
            {warnings.length > 0 && (
              <span className="text-amber-500">
                {warnings.length} {warnings.length === 1 ? t("company.exportPage.warning") : t("company.exportPage.warnings")}
              </span>
            )}
          </div>
          <Button
          ...
          >
            ...
            {downloadMutation.isPending
              ? t("company.exportPage.buildingExport")
              : t("company.exportPage.exportFiles", { count: selectedCount })}
          </Button>
        </div>
      </div>
      ...
      <div className="grid h-[calc(100vh-12rem)] gap-0 xl:grid-cols-[19rem_minmax(0,1fr)]">
        <aside className="flex flex-col border-r border-border overflow-hidden">
          <div className="border-b border-border px-4 py-3 shrink-0">
            <h2 className="text-base font-semibold">{t("company.exportPage.packageFiles")}</h2>
          </div>
          <div className="border-b border-border px-3 py-2 shrink-0">
            <div className="flex items-center gap-2 rounded-md border border-border px-2 py-1">
              ...
              <input
                ...
                placeholder={t("company.exportPage.searchFiles")}
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {totalTaskChildren > visibleTaskChildren && !treeSearch && (
              <div className="px-4 py-2">
                ...
                >
                  {t("company.exportPage.showMoreIssues", { visible: visibleTaskChildren, total: totalTaskChildren })}
                </button>
              </div>
            )}
          </div>
