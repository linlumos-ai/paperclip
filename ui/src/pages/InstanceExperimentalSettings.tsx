  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          <DialogTitle>{t("instance.experimentalPage.recoveryDialog.title")}</DialogTitle>
          <DialogDescription>
            {preview
              ? `${count} ${count === 1 ? "recovery task" : "recovery tasks"} match the last ${preview.lookbackHours} hours.`
              : t("instance.experimentalPage.recoveryDialog.checkingCandidates")}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="max-h-[min(28rem,65vh)] space-y-3 overflow-y-auto pr-1">
          {preview && preview.items.length === 0 ? (
            <div className="rounded-md border border-border bg-muted/30 px-3 py-4 text-sm text-muted-foreground">
              {t("instance.experimentalPage.recoveryDialog.noRecoveryTasks")}
            </div>
          ) : null}
          ...
          {preview?.items.map((item) => (
            <div key={item.incidentKey} className="rounded-md border border-border bg-card px-3 py-3">
              ...
              <div className="mt-2 text-xs text-muted-foreground">
                {t("instance.experimentalPage.recoveryDialog.recoveryTarget")}{" "}
                <a
        ...
        {preview && preview.skippedOutsideLookback > 0 ? (
          <p className="text-xs text-muted-foreground">
            {t("instance.experimentalPage.recoveryDialog.outsideLookback", {
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)} disabled={isPending}>
            {t("instance.experimentalPage.recoveryDialog.cancel")}
          </Button>
          <Button variant="outline" onClick={onEnableOnly} disabled={isPending || !preview}>
            {t("instance.experimentalPage.recoveryDialog.enableOnly")}
          </Button>
          <Button onClick={onEnableAndRun} disabled={isPending || !preview}>
            {count > 0
              ? t("instance.experimentalPage.recoveryDialog.enableAndCreate", { count })
              : t("instance.experimentalPage.recoveryDialog.enable")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export function InstanceExperimentalSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.experimentalPage.failedToUpdate"));
    },
  });
  ...
  const previewMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.experimentalPage.failedToPreviewRecovery"));
    },
  });
  ...
  const runRecoveryMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.experimentalPage.failedToCreateRecovery"));
    },
  });
  ...
  if (experimentalQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instance.experimentalPage.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {experimentalQuery.error instanceof Error
          ...
          : t("instance.experimentalPage.loadError")}
      </div>
    );
  }
  ...
  function previewForEnable() {
    if (!lookbackHoursIsValid) {
      setActionError(t("instance.experimentalPage.lookbackHoursInvalid"));
      return;
    }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instance.experimentalPage.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">
          {t("instance.experimentalPage.description")}
        </p>
      </div>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instance.experimentalPage.enableEnvironments")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("instance.experimentalPage.enableEnvironmentsDesc")}
            </p>
          </div>
          <ToggleSwitch
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instance.experimentalPage.enableIsolatedWorkspaces")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("instance.experimentalPage.enableIsolatedWorkspacesDesc")}
            </p>
          </div>
          <ToggleSwitch
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("instance.experimentalPage.autoRestartDevServer")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("instance.experimentalPage.autoRestartDevServerDesc")}
            </p>
          </div>
          <ToggleSwitch
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex flex-col gap-5">
          <div className="flex items-start justify-between gap-4">
            <div className="space-y-1.5">
              <h2 className="text-sm font-semibold">{t("instance.experimentalPage.autoCreateRecoveryTasks")}</h2>
              <p className="max-w-2xl text-sm text-muted-foreground">
                {t("instance.experimentalPage.autoCreateRecoveryTasksDesc")}
              </p>
            </div>
            <ToggleSwitch
          ...
          <div className="grid gap-3 sm:grid-cols-[minmax(10rem,14rem)_1fr] sm:items-end">
            <label className="space-y-1.5">
              <span className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
                ...
                {t("instance.experimentalPage.lookbackHours")}
              </span>
              <Input
                type="number"
                min={1}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                {t("instance.experimentalPage.saveHours")}
              </Button>
              <Button
                variant="outline"
              ...
              >
                ...
                {t("instance.experimentalPage.preview")}
              </Button>
              <Button
                onClick={() => {
                  if (!lookbackHoursIsValid) {
              ...
              >
                ...
                {t("instance.experimentalPage.runNow")}
              </Button>
            </div>
          </div>

          <p className="text-xs text-muted-foreground">
            {t("instance.experimentalPage.currentWindow", { hours: lookbackHours })}
          </p>
        </div>
      </section>
