...
export function JoinRequestQueue() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("toast.joinRequestApproved"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("toast.joinRequestRejected"), tone: "success" });
    },
  });
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("joinRequestQueue.title")}</h1>
        </div>
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("joinRequestQueue.description")}
        </p>
      </div>

      <div className="flex flex-wrap gap-3 rounded-xl border border-border bg-card p-4">
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("joinRequestQueue.status")}</span>
          <select
          ...
          >
            <option value="pending_approval">{t("joinRequestQueue.pendingApproval")}</option>
            <option value="approved">{t("joinRequestQueue.approved")}</option>
            <option value="rejected">{t("joinRequestQueue.rejected")}</option>
          </select>
        </label>
        <label className="space-y-2 text-sm">
          <span className="font-medium">{t("joinRequestQueue.requestType")}</span>
          <select
            className="rounded-md border border-border bg-background px-3 py-2"
            value={requestType}
            onChange={(event) =>
                  ...
                  <div>
                    <div className="text-base font-medium">
                      {request.requestType === "human"
                        ? request.requesterUser?.name || request.requestEmailSnapshot || request.requestingUserId || t("common.unknown")
                        : request.agentName || t("joinRequestQueue.pending")}
                    </div>
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  ...
                  <div className="mt-2">
                    {request.invite
                      ...
                      : t("joinRequestQueue.inviteUnavailable")}
                  </div>
