...
export function NotFoundPage({ scope = "global", requestedPrefix }: NotFoundPageProps) {
  ...
  useEffect(() => {
    setBreadcrumbs([{ label: t("common.notFound") }]);
  }, [setBreadcrumbs, t]);
  ...
  const title = scope === "invalid_company_prefix" ? t("common.companyNotFound") : t("common.notFound");
  const description =
    scope === "invalid_company_prefix"
      ? t("common.noCompanyMatchesPrefix", { prefix: normalizedPrefix ?? "unknown" })
      : t("common.routeNotExist");
    ...
    <div className="mx-auto max-w-2xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4 rounded-md border border-border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          {t("common.requestedPath")} <code className="font-mono">{currentPath}</code>
        </div>
