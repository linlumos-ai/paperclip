...
export function PluginPage() {
  ...
  useEffect(() => {
    if (pageSlot) {
      setBreadcrumbs([
        { label: t("nav.plugins"), href: "/instance/settings/plugins" },
  ...
  if (!resolvedCompanyId) {
    ...
    return (
      <div className="space-y-4">
        <p className="text-sm text-muted-foreground">{t("plugins.selectCompany")}</p>
      </div>
    );
  }
    ...
    if (duplicateMatches.length > 1) {
      return (
        <div className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-sm text-destructive">
          Multiple plugins declare the route <code>{pluginRoutePath}</code>. {t("plugins.multiplePluginsDeclareRoute")}
        </div>
      );
    }
  }
